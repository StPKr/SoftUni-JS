...
const createTemp = ({ name, img, ingredients, steps }, errors, onCreate) => html`
<h1>Create Recipe</h1>
...
function update(data = {}, errors = {}) {
    render(createTemp(data, errors, createSubmitHandler(onCreate)));
}

async function onCreate({ name, img, ingredients, steps }, form) {
    ingredients = ingredients.split('\n').filter(l => !!l); //only include lines with text on them
    steps = steps.split('\n').filter(l => !!l);
    ...
    //Disable form - user can spam button and create 20 of the same recipe
